...
def scrape_npr(max_articles_per_category):
    ...
    relativeURLs = ['Business', 'Politics', 'Science', 'Health',  'Technology', "World"]
    ...
    for URL in relativeURLs:
        try:
            ...
            # Find containers
            containers = response.html.find('#main-section')
            links = []
            ...
            for index, link in enumerate(links):
                ...
                if "https://" in link or "http://" in link:
                    if link.startswith(start_url):
                        #if link is followed by number
                        link_check = link.replace(start_url, "")[0]
                        if (link_check.isdigit()):
                            #condition for top link
                            if index <= 15:
    ...
    for link in curatedLinks:
        ...
        try:
            ...
            results = soup.find(class_='storytext')
            ...
            imageContainer = results.find(class_='imagewrap')
